...
int T[MAX][MAX][2] = {}, edge_sort[MAX*3] = {}, edge[MAX*3] = {}, A[MAX] = {}, column = 3, wc_num = 3, set =0, opy_num = 0, nd1 = 0, nd2 = 0, w = 0, total_weight = 0; //opt_num: option, column: merge sort column #, wc: weight column #
	...
	for(int i=0;i<MAX;i++){ 
		...
		if( -1 != T[node][i][0] && -1 != A[i]){   
			if( T[node][i][1] < edge[*edge_num_ptr+2] ){ //comparison of weight
				...
				*edge_num_ptr += 1; //count number of edge
				next = i; //temp register for # of next node
			}
		}
